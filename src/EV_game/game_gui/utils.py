...
from src.gui.utils.constants import game_font, SCREEN, BEIGE, GREEN, GAME_BG, cards_object
...
def recapRound(list_winner, common_cards=None):
    ...
    if len(list_winner) == 1:
        ...
        y_c = y_coordinate(HEIGHT, font.get_height(), font.get_height() // 2, HEIGHT * 0.1, len(list_winner))
        for i in range(len(list_winner)):
            text = '{} won ${} with {}'.format(str(list_winner[i][0].name),
                                               str(list_winner[i][1]),
...
def giveCard(type_card, cards):
    ...
    # Check if cards is a list and has enough elements; if not, skip drawing cards.
    if not isinstance(cards, list) or len(cards) < len(list_cards):
        return sub_cards
    from src.gui.utils.constants import cards_object
    for i in range(len(list_cards)):
        card_code = cards[i]
        if card_code in cards_object:
            card_object = cards_object[card_code]
...
def recapRoundEV(ev_change, round_number):
    ...
    import pygame, time
    font = game_font(20)
    # Use GREEN if EV is non-negative, else red
