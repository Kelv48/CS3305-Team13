...
class SetEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, set) or isinstance(obj, ServerConnection):
            return list(obj)
...
async def der(websocket):
    logger.info
    message = template.substitute(m_type=Protocols.Response.LIST_OF_GAMES, data=json.dumps(activeSessions, cls=SetEncoder))
    ...
    while True:
        try:
            ...
            match message['m_type']:
                ...
                case Protocols.Request.LIST_OF_GAMES:
                    await der(websocket)
